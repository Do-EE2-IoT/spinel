            ...
            try:
                ...
                if pkt.upper_layer_protocol.type == ipv6.IPV6_NEXT_HEADER_ICMP:
                    ...
                    if udp_pkt.header.dst_port == COAP_PORT:
                        ...
                        # PUMP GET response
                        elif coap_packet_type == "PUMP_GET_RESP":
                            if len(p.payload) == 3:
                                pump1_state = "Turn Off" if int(p.payload[0]) == 0 else "Turn On"
                                pump2_state = "Turn Off" if int(p.payload[1]) == 0 else "Turn On"
                                pump3_state = "Turn Off" if int(p.payload[2]) == 0 else "Turn On"
                                print("PUMP NUMBER 1 is : {},PUMP NUMBER 2 is: {},PUMP NUMBER 3 is: {}".format(pump1_state, pump2_state,pump3_state))
                            else:
                                print("Error: invalid PUMP state")
                        # LED PUT/POST response
                        elif coap_packet_type == "LED_SET_RESP":
                            print("LED state successfully set")
                        elif coap_packet_type == "PUMP_SET_RESP":
                            print("PUMP state successfully set")
                ...
                for i in range(4, len(params)):
                    ...
                    if params[i] == '--led_state' and len(params) >= (i + 2):
                        ...
                        if params[i+2] == '0':
                            ...
                            return
                    if params[i] == '--pump_state' and len(params) >= (i + 3):
                        if params[i+1] == 'pump1':
                            led_target = COAP_PUM1_ID
                        elif params[i+1] == 'pump2':
                            led_target = COAP_PUM2_ID
                        elif params[i+1] == 'pump3':
                            led_target = COAP_PUM3_ID
                        else:
                            print("Invalid PUMP target, must be 1 or 2 or 3")
                            return
                        ...
                        if params[i+2] == '100':
                            ...
                            led_state = 5
                        elif params[i+2] == '1000':
                            led_state = 10
                        elif params[i+2] == '2000':
                            led_state = 20
